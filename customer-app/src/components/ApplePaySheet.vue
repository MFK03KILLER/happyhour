      ...
      <div class="relative w-full max-w-md bg-white rounded-t-3xl shadow-lift animate-slide-up pb-[max(env(safe-area-inset-bottom),24px)]">
        ...
        <div v-if="stage==='idle'" class="px-6 pt-4 pb-2">
          <div class="text-center mb-4">
            <div class="text-xs uppercase tracking-wider text-ink-300 font-semibold">Pay {{ merchantName }}</div>
            <div class="text-3xl font-bold mt-1">${{ amount.toFixed(2) }}</div>
          ...
          <div class="space-y-3">
            ...
            <button @click="pay('card')" class="w-full bg-cream-200 text-ink-900 rounded-2xl py-4 flex items-center justify-center gap-2 font-semibold active:scale-[.98] transition">
              ...
              Pay with card · Amex •••• 0005
            </button>
          </div>

          <div class="mt-4 text-center text-[11px] text-ink-300">
            Demo payment — no real charge. <span class="font-semibold">Mock mode</span>
          </div>
        </div>
        ...
        <div v-else-if="stage==='processing'" class="px-6 pt-8 pb-12 text-center">
          ...
          <div class="mt-5 font-semibold">Authorizing with Face ID</div>
          <div class="text-sm text-ink-500 mt-1">Hold your device near to continue</div>
        </div>
        ...
        <div v-else class="px-6 pt-8 pb-12 text-center animate-fade-in">
          ...
          <div class="mt-5 text-xl font-bold">Payment successful</div>
          <div class="text-sm text-ink-500 mt-1">Your coupon is now in your Wallet.</div>
        </div>
      </div>
    </div>
  </teleport>
