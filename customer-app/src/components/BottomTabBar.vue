...
<template>
  <nav
    class="fixed bottom-0 left-0 right-0 z-40 px-4 pb-[max(env(safe-area-inset-bottom),16px)] pt-2 pointer-events-none"
  >
    <div class="mx-auto max-w-md glass rounded-full shadow-lift border border-white/40 px-2 py-2 flex items-center justify-between pointer-events-auto">
      ...
      >
        ...
        <svg v-if="t.icon==='orders'" class="w-6 h-6" :fill="isActive(t.to) ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path d="M4 6h16v4H4z"/><path d="M5 10v9a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-9"/><path d="M10 14h4"/></svg>
