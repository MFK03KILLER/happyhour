  ...
  <div class="pb-28 safe-top">
    <header class="px-5 pt-6">
      ...
      <div class="mt-4 relative">
        <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-ink-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path stroke-linecap="round" d="m20 20-3.5-3.5"/></svg>
        <input
          v-model="search"
    ...
    <div class="mt-4 flex gap-2 px-5 overflow-x-auto scroll-no-bar">
      <button
        ...
        :class="!category ? 'bg-ink-900 text-white' : 'bg-white border border-ink-300/30 text-ink-700'"
      >All</button>
      <button
        ...
        @click="setCat(c.slug)"
        class="flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition"
        :class="category===c.slug ? 'bg-ink-900 text-white' : 'bg-white border border-ink-300/30 text-ink-700'"
      >{{ c.name }}</button>
    </div>
    ...
    <section class="mt-6 px-5 space-y-4">
      ...
      <div v-else-if="items.length === 0" class="text-center py-12 text-ink-500">
        ...
        <div>No offers match — try a different search.</div>
      </div>
