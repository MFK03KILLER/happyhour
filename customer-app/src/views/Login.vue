...
onMounted(async () => {
  try {
    const { data } = await client.get('/public/terms', { params: { audience: 'consumer' } });
...
async function verifyOtp() {
  ...
  try {
    ...
    if (code === 'FULL_NAME_REQUIRED' || msg.includes('نام کامل')) {
  ...
  <div class="min-h-screen flex flex-col safe-top px-6 pb-10">
    ...
    <form v-if="stage === 'phone'" @submit.prevent="sendOtp" class="mt-10 space-y-3 max-w-sm mx-auto w-full">
      ...
      <p class="text-xs text-ink-500 text-center pt-2">
        با ورود، با
        <button type="button" @click="showTerms = true" class="text-teal-700 font-semibold underline">قوانین و شرایط</button>
        هپی‌اَور موافقت می‌کنید.
        <span v-if="termsVersion" class="text-ink-300">(نسخه {{ termsVersion }})</span>
      </p>
    </form>
