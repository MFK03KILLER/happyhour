...
onMounted(async () => {
  await flags.load();
  if (!flags.isOn('maps')) {
    error.value = 'Maps view is not enabled yet.';
    loading.value = false;
    return;
  }
  try {
...
<template>
  <div class="fixed inset-0 bg-ink-900">
    <div v-if="loading" class="absolute inset-0 flex items-center justify-center">
      <div class="text-white text-sm">Loading map…</div>
    </div>
    ...
    <div v-else-if="error" class="absolute inset-0 flex items-center justify-center px-6">
      <div class="text-center text-white">
        ...
        <p class="text-sm opacity-80 mt-1">Ask the admin to enable Maps.</p>
        <button @click="router.back()" class="mt-4 ios-button-secondary text-ink-900">Go back</button>
      </div>
    </div>

    <LeafletMap v-else :pins="pins" :dark="true" :zoom="11" @pin-click="onPin" />
    ...
    <header v-if="!loading && !error" class="absolute top-0 inset-x-0 z-[1000] safe-top p-3 flex items-center gap-2 pointer-events-none">
      ...
      <div class="bg-white rounded-full shadow-lift px-4 py-2 flex-1 max-w-xs pointer-events-auto">
        <div class="text-[10px] uppercase tracking-wider text-ink-500 font-semibold">Nearby</div>
        <div class="text-sm font-bold">{{ merchants.length }} locations · {{ surpriseBags.length }} bags</div>
      </div>
    </header>

    <button v-if="!loading && !error" @click="showList = !showList"
      class="absolute bottom-[max(env(safe-area-inset-bottom),16px)] left-1/2 -translate-x-1/2 z-[1000] bg-ink-900 text-white rounded-full px-5 py-3 font-semibold shadow-lift active:scale-95 flex items-center gap-2">
      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex gap-1.5 mt-1.5">
            <span v-if="selectedPin.perks" class="chip bg-teal-50 text-teal-700 text-[10px]">{{ selectedPin.perks }} perks</span>
            <span v-if="selectedPin.bags" class="chip bg-coral-500/10 text-coral-600 text-[10px]">🔥 {{ selectedPin.bags }} bags</span>
          </div>
        </div>
        <button @click="directions(selectedPin.merchant)" class="bg-teal-600 text-white rounded-full px-3 py-2 text-xs font-bold active:scale-95">Directions</button>
      </div>
      <button @click="selectedPin = null" class="absolute top-2 right-2 text-ink-300 active:opacity-50">✕</button>
    </div>
    ...
    <div v-if="showList" class="absolute inset-x-0 bottom-0 z-[1002] max-h-[60vh] bg-white rounded-t-3xl shadow-lift overflow-hidden animate-slide-up">
      <div class="px-5 pt-3 pb-2 sticky top-0 bg-white border-b border-cream-200">
        ...
        <div class="font-bold">{{ merchants.length }} nearby</div>
      </div>
      <div class="overflow-y-auto pb-[max(env(safe-area-inset-bottom),20px)]" style="max-height:50vh">
        <div v-for="entry in merchants" :key="entry.merchant._id"
          class="px-5 py-3 flex items-center gap-3 border-b border-cream-200 active:bg-cream-100"
          @click="selectedPin = entry; showList = false">
          ...
          <div class="flex gap-1.5">
            <span v-if="entry.bags" class="chip bg-coral-500/10 text-coral-600 text-[10px]">🔥 {{ entry.bags }}</span>
            <span v-if="entry.perks" class="chip bg-teal-50 text-teal-700 text-[10px]">{{ entry.perks }}</span>
          </div>
        </div>
      </div>
    </div>
