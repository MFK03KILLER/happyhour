...
<template>
  <div class="min-h-screen bg-cream-100 safe-top pb-12">
    <header class="px-5 pt-4 flex items-center">
      <button @click="router.push('/orders')" class="w-10 h-10 -ml-2 rounded-full flex items-center justify-center active:bg-cream-200">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
      </button>
      <div class="flex-1 text-center font-semibold -ml-8">Your order</div>
    </header>
    ...
    <div v-else-if="order" class="px-5 mt-4 space-y-5">
      <div class="rounded-3xl bg-gradient-to-br from-teal-500 to-teal-700 text-white p-5 shadow-lift relative overflow-hidden">
        ...
        <div class="relative text-center">
          <div class="font-semibold text-base">How was your overall experience?</div>
        ...
        <div class="p-5">
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-sm text-ink-500 truncate">{{ order.merchantId?.address?.street }}, {{ order.merchantId?.address?.city }}, {{ order.merchantId?.address?.state }}</div>
            </div>
          </div>

          <div class="mt-5 grid grid-cols-2 gap-y-5 gap-x-4">
            <div>
              <div class="text-[11px] uppercase tracking-wider text-ink-500 font-semibold">Picked up</div>
              <div class="font-semibold mt-0.5">{{ formatDate(order.scannedAt) }}</div>
            </div>
            <div>
              <div class="text-[11px] uppercase tracking-wider text-ink-500 font-semibold">Order ID</div>
              <div class="font-mono font-semibold mt-0.5">{{ shortId(order._id) }}</div>
            </div>
            <div>
              <div class="text-[11px] uppercase tracking-wider text-ink-500 font-semibold">Surprise bag</div>
              <div class="font-semibold mt-0.5">1× {{ order.couponId?.title }}</div>
            </div>
            <div class="text-right">
              <div class="text-[11px] uppercase tracking-wider text-ink-500 font-semibold">Total</div>
              <div class="font-semibold mt-0.5">${{ (order.amountSavedUSD || order.couponId?.priceUSD || 0).toFixed(2) }}</div>
            </div>
            <div class="col-span-2">
              <div class="text-[11px] uppercase tracking-wider text-ink-500 font-semibold">Payment method</div>
              <div class="font-semibold mt-0.5">Apple Pay: Amex</div>
            </div>
          </div>
        </div>
      </div>
