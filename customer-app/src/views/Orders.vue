...
<template>
  <div class="pb-28 safe-top">
    <header class="px-5 pt-6">
      ...
      <p class="text-ink-500 mt-1">Your past redemptions.</p>
    </header>
    ...
    <div v-else-if="items.length === 0" class="px-5 mt-16 text-center text-ink-500">
      No orders yet — redeem your first coupon to see it here.
    </div>
    <div v-else class="px-5 mt-6 space-y-3">
      <router-link
        ...
        :key="o._id"
        :to="`/orders/${o._id}`"
        class="block ios-card p-4 active:scale-[.99] transition"
      >
        <div class="flex items-center gap-3">
          ...
          <div class="text-right">
            <div class="text-[11px] text-ink-300">{{ new Date(o.scannedAt).toLocaleDateString('en-US', { month:'short', day:'numeric'}) }}</div>
            <div class="text-sm font-semibold text-teal-700">${{ (o.amountSavedUSD || 0).toFixed(2) }}</div>
          </div>
        </div>
      </router-link>
    </div>
