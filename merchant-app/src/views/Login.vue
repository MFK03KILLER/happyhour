...
const email = ref('pizza.staff@happyhour.demo');
const password = ref('Merchant@123');
...
<template>
  <div class="min-h-screen bg-gradient-to-br from-teal-700 via-teal-800 to-ink-900 text-white safe-top px-6 pb-10 flex flex-col">
    <div class="pt-12 flex flex-col items-center text-center">
      ...
      <h1 class="mt-5 text-3xl font-bold tracking-tight">Merchant console</h1>
      <p class="mt-1.5 text-cream-50/80">Scan customer QRs and grow your business.</p>
    </div>

    <form @submit.prevent="submit" class="mt-10 space-y-3 max-w-sm mx-auto w-full">
      <input v-model="email" type="email" class="input bg-white/15 text-white border-white/20 placeholder:text-white/50 focus:border-coral-500" placeholder="Work email" required />
      <input v-model="password" type="password" class="input bg-white/15 text-white border-white/20 placeholder:text-white/50 focus:border-coral-500" placeholder="Password" required />
    ...
    <div class="mt-auto text-center text-[11px] text-white/60">
      Demo creds prefilled · merchant: pizza.staff@happyhour.demo · admin: admin@happyhour.demo
    </div>
  </div>
</template>
