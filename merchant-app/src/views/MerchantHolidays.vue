    ...
    <header class="bg-gradient-to-br from-teal-700 to-teal-800 text-white safe-top px-5 pb-8 rounded-b-3xl">
      <div class="pt-2 flex items-center justify-between">
        <div>
          ...
          <h1 class="text-2xl font-bold mt-0.5">Holidays</h1>
        </div>
      ...
      <p class="mt-3 text-sm opacity-90">Coupons with "Disable on holidays" turned on (default) will be unavailable on these dates.</p>
    </header>
    ...
    <div class="px-5 -mt-6 space-y-4">
      <div v-if="todayInfo" class="ios-card p-4 border-2 border-coral-500/40 bg-coral-500/5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="font-bold text-coral-600">Today is a holiday</div>
            <div class="text-sm text-ink-700">{{ todayInfo.name }} — holiday-disabled coupons are paused.</div>
          </div>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div class="ios-card p-5">
          <div class="flex items-center justify-between">
            <div>
              <div class="font-bold">Your custom holidays</div>
              <div class="text-xs text-ink-500">Dates you mark as closed for this location.</div>
            </div>
          ...
          <form v-if="showAdd && can('manage_hours')" @submit.prevent="addCustom" class="mt-3 space-y-2">
            ...
            <input v-model="newHoliday.name" class="input" placeholder="Name (e.g. Owner's birthday, Eid)" required maxlength="60" />
            <div class="flex gap-2">
              <button type="button" @click="showAdd = false" class="ios-card flex-1 py-2.5 font-semibold text-ink-700">Cancel</button>
              <button type="submit" :disabled="saving" class="ios-button-primary flex-1">{{ saving ? 'Adding…' : 'Add holiday' }}</button>
            </div>
          </form>

          <div v-if="customs.length === 0" class="mt-3 text-sm text-ink-500 italic">No custom holidays yet.</div>
        ...
        <div class="ios-card p-5">
          <div class="font-bold">US federal holidays · {{ year }}</div>
          <p class="text-xs text-ink-500 mt-1">Automatic. If you DO want a coupon active on a federal holiday, edit the coupon and turn off "Disable on holidays".</p>
          <div v-if="globals.length === 0" class="mt-3 text-sm text-ink-500 italic">No globals seeded yet — run the seed script on server.</div>
          <div v-else class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
            <div v-for="h in globals" :key="h._id" class="flex items-center justify-between py-2 px-3 rounded-2xl bg-cream-100">
              ...
              <span class="chip bg-teal-50 text-teal-700 text-[10px]">Federal</span>
            </div>
          </div>
        </div>

        <button @click="doLogout" class="ios-card w-full p-4 text-coral-600 font-semibold active:bg-cream-100 text-center">Sign out</button>
      </template>
    </div>
  </div>
</template>
